...
void loadSales(GtkBuilder *builder, GtkWidget *mainWindow) {
	...
	accounts->invoiceNumber = g_strdup(NULL);
    ...
    accountCloseButton = GTK_WIDGET(gtk_builder_get_object(builder, "closeButton"));
   	...
   	accounts->numberOfSalesLabel = GTK_WIDGET(gtk_builder_get_object(builder, "numberOfSales"));
	...
	g_signal_connect(G_OBJECT(accounts->searchEntry), "activate", G_CALLBACK(getSales), accounts);
	
	GtkWidget *searchButton = GTK_WIDGET(gtk_builder_get_object(builder, "searchButton"));
	g_signal_connect(G_OBJECT(searchButton), "clicked", G_CALLBACK(getSales), accounts);
	...
	accounts->viewButton = GTK_WIDGET(gtk_builder_get_object(builder, "viewButton"));
	gtk_widget_set_sensitive(accounts->viewButton, FALSE);
	g_signal_connect(G_OBJECT(accounts->viewButton), "clicked", G_CALLBACK(prepareLoadInvoice), accounts);	

	/* Load the inventory tree with all data from the database */
	//getAccounts(NULL, accounts);
	g_signal_connect(accounts->mainWindow, "show", G_CALLBACK(getSales), accounts);
}

/* Load up the invoice -> saleView.c */
static void prepareLoadInvoice(GtkWidget *widget, intrackAccounts *accounts) {
	loadSaleView(accounts->mainWindow, accounts->invoiceNumber, accounts->selectedAccount); /* -> saleView.c */
}
...
static void calculateTreeTotals(GtkListStore *store, intrackAccounts *accounts) {
	...
	if(gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter)) {
		...
		/* Parts Sales */
		gtk_tree_model_get(GTK_TREE_MODEL (store), &iter, SALE_TOTAL, &rowtext, -1);
	...
	gchar *numberOfSalesChar = NULL;
	gchar *partSalesChar = NULL;
	
	numberOfSalesChar = g_strconcat("Number Of Sales: ", g_strdup_printf("%i", numberOfSales), NULL);
	partSalesChar = g_strconcat("Total Sales: ", g_strdup_printf("%.2f", partSales), NULL);

	gtk_label_set_text(GTK_LABEL(accounts->numberOfSalesLabel), numberOfSalesChar);
	gtk_label_set_text(GTK_LABEL(accounts->totalSalesLabel), partSalesChar);

	g_free(numberOfSalesChar), g_free(partSalesChar);
}

/* Prepare to pull the account data out of the database */
static void getSales(GtkWidget *widget, intrackAccounts *accounts) {
	GtkListStore *store;

	store = gtk_list_store_new(COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_FLOAT);
	...
	if(strlen(gtk_entry_get_text(GTK_ENTRY(accounts->searchEntry))) > 2) {
		...
		pullSales(store, accounts, searchString);
...
static int pullSales(GtkListStore *store, intrackAccounts *accounts, gchar *searchString) {
	GtkTreeIter 	iter;
	gchar *query_string;
	int query_state;
	int i;
    ...
    // Select the database.
    query_string = g_strdup(SALES_DATABASE);
	...
	/*	
	query_string = g_strconcat("SHOW TABLES FROM ", SALES_DATABASE, NULL);
	...
	*/
		//query_string = "SELECT * FROM `" . $row[0] . "` WHERE partNo LIKE '%" . $search . "%' ORDER BY id ASC";
		if(searchString != NULL)		
			query_string = g_strconcat("SELECT id, email, name, company, address, city, province, country, code, phone, fax, notes, orderPO, orderDate, saleNo, userID, orderTotal FROM `", SALES_TABLE, "` WHERE name LIKE '%", searchString, "%' OR email LIKE '%", searchString, "%' OR company LIKE '%", searchString, "%' OR address LIKE '%", searchString, "%' OR city LIKE '%", searchString, "%' OR province LIKE '%", searchString, "%' OR city LIKE '%", searchString, "%' OR country LIKE '%", searchString, "%' OR code LIKE '%", searchString, "%' OR orderPO LIKE '%", searchString, "%' OR saleNo LIKE '%", searchString, "%' ORDER BY id ASC", NULL);
		else
			query_string = g_strconcat("SELECT id, email, name, company, address, city, province, country, code, phone, fax, notes, orderPO, orderDate, saleNo, userID, orderTotal FROM `", SALES_TABLE, "` ORDER BY id ASC", NULL);
		...
		while ((accountRow = mysql_fetch_row(accountResult))) {
			gchar *saleNo, *orderDate, *email, *name, *company, *address, *city, *province, *country, *code, *phone, *fax, *notes, *orderPO, *account;
			gchar *saleTotal;
			float totalFloat = 0.00;
			//account = g_strdup(row[0]);
			
			for(i = 0; i < num_fields; i++) {
				if(i == 1)
					email = g_strdup(accountRow[i]);
					
				if(i == 2)
					name = g_strdup(accountRow[i]);
					
				if(i == 3)
					company = g_strdup(accountRow[i]);
					
				if(i == 4)
					address = g_strdup(accountRow[i]);
					
				if(i == 5)
					city = g_strdup(accountRow[i]);
					
				if(i == 6)
					province = g_strdup(accountRow[i]);
					
				if(i == 7)
					country = g_strdup(accountRow[i]);
					
				if(i == 8)
					code = g_strdup(accountRow[i]);
					
				if(i == 9)
					phone = g_strdup(accountRow[i]);
					
				if(i == 10)
					fax = g_strdup(accountRow[i]);
					
				if(i == 11)
					notes = g_strdup(accountRow[i]);
					
				if(i == 12)
					orderPO = g_strdup(accountRow[i]);
					
				if(i == 13)
					orderDate = g_strdup(accountRow[i]);
					
				if(i == 14)
					saleNo = g_strdup(accountRow[i]);
					
				if(i == 15)
					account = g_strdup(accountRow[i]);	
					
				if(i == 16) {
					saleTotal = g_strdup(accountRow[i]);										
					totalFloat = atof(accountRow[i]);
				}
			}
			...
			/* Calculate the total sales for the invoice */
			/*
			gchar *saleTotal;
			gdouble totalSalesDouble = 0.00;
			totalSalesDouble = getTotalSales(account, saleNo, accounts->mainWindow);
			saleTotal = g_strdup_printf("%.2f", totalSalesDouble);
			*/
			
			gtk_list_store_set(store, &iter, ORDER, saleNo, SALE_TOTAL, saleTotal, ORDER_DATE, orderDate, EMAIL, email, NAME, name, COMPANY, company, ADDRESS, address, CITY, city, PROVSTATE, province, COUNTRY, country, CODE, code, PHONE, phone, FAX, fax, NOTES, notes, ORDER_PO, orderPO, ACCOUNT, account, TOTALF, totalFloat, -1);

			g_free(email), g_free(name), g_free(company), g_free(address), g_free(city), g_free(province), g_free(country), g_free(code);
			g_free(phone), g_free(fax), g_free(notes), g_free(orderPO), g_free(orderDate), g_free(account), g_free(saleTotal), g_free(saleNo);
		}
...
static void setupWarrantyTree(intrackAccounts *accounts) {
	...
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_reorderable(column, TRUE);
	gtk_tree_view_column_set_sort_column_id (column, TOTALF); 
	gtk_tree_view_append_column (GTK_TREE_VIEW (accounts->accountTree), column);
	
	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes("Order Date", renderer, "text", ORDER_DATE, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_reorderable(column, TRUE);
	gtk_tree_view_column_set_sort_column_id (column, ORDER_DATE); 
	gtk_tree_view_append_column (GTK_TREE_VIEW (accounts->accountTree), column);	
	...
	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes("PO #", renderer, "text", ORDER_PO, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_reorderable(column, TRUE);
	gtk_tree_view_column_set_sort_column_id (column, ORDER_PO); 
	gtk_tree_view_append_column (GTK_TREE_VIEW (accounts->accountTree), column);		
	...
	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes("Total Float", renderer, "text", TOTALF, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_column_set_reorderable(column, TRUE);
...
static gdouble getTotalSales(gchar *accountNo, gchar *saleNo, GtkWidget *mainWindow) {
	...
	gchar *saleTotal;
	saleTotal = g_strdup_printf("%.2f", totalSales);
				
	query_string = g_strconcat("UPDATE `sales`", " SET orderTotal='", saleTotal, "' WHERE saleNo='", saleNo, "'", NULL);
	query_state=mysql_query(accountConnection, query_string);
