...
//		Revision: 08-12-2014: modified to work with milparts parent category id settings
...
#include "settings.h"
#include "messages.h"
#include "category_popup.h"


void categoryPopup(GtkWidget *parentWindow, gchar *itemCode, GtkWidget *parentTree, gboolean b_inventory) {
	...
	intrackCat->selectedCat = g_strdup(NULL);
	intrackCat->selectedCatName = g_strdup(NULL);
	intrackCat->selectedSection = g_strdup(NULL);
	intrackCat->selectedSectionName = g_strdup(NULL);
	intrackCat->b_inventory = b_inventory;
    ...
    closeButton = GTK_WIDGET(gtk_builder_get_object(builder, "cancelButton"));
   	...
   	g_signal_connect(G_OBJECT(intrackCat->applyButton), "clicked", G_CALLBACK(changeCat), intrackCat);
   	...
   	g_signal_connect(G_OBJECT(noCatButton), "clicked", G_CALLBACK(noCatConfirm), intrackCat);
	...
	setupCategoryTree(intrackCat); // Setup the tree
	
	getCategories(intrackCat, itemCode); // Load the tree with the category data from the database
...
static void catFreeMemory(GtkWidget *widget, IntrackCategories *intrackCat) {
	...
	g_free(intrackCat->selectedCatName);
...
static void noCatConfirm(GtkWidget *widget, IntrackCategories *intrackCat) {
	
	GtkWidget *dialog;
	gint widgetDialog;
	gchar *message;
	
	if(intrackCat->b_inventory == TRUE)
		message = g_strdup("Set item to not belong in any categories ?");
	else
		message = g_strdup("Set category to be a root category ?");
	...
	gtk_window_set_title(GTK_WINDOW(dialog), "Set to root Category?");
...
static void changeCat(GtkWidget *widget, IntrackCategories *intrackCat) {
	if(intrackCat->selectedCat == NULL && widget != NULL)
		printMessage("ERROR: Please select a category.");
	else {
		int update_state;
		gchar *query;
		
		// If we update category via inventory.c
		if(intrackCat->b_inventory == TRUE) {
			databaseEditItemByID(mysqlDatabase, mysqlTables, intrackCat->itemCode, intrackCat->selectedCat, "category");
			...
			if(intrackCat->selectedCat == NULL)
				query = g_strconcat("UPDATE `", CATEGORY_TABLES, "` SET parent_id='0' WHERE id='", intrackCat->itemCode, "'", NULL);
			else
				query = g_strconcat("UPDATE `", CATEGORY_TABLES, "` SET parent_id='", intrackCat->selectedCat, "', section ='", intrackCat->selectedSection, "' WHERE id='", intrackCat->itemCode, "'", NULL);
...
static void parentTreeGetRow(GtkTreeRowReference *ref, IntrackCategories *intrackCat) {
	...
	if(!strcmp(idcode, intrackCat->itemCode)) {
		...
		// For updating inventory categories from inventory.c
		if(intrackCat->b_inventory == TRUE) {
			if(intrackCat->selectedCat == NULL)
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, 5, "", -1); // 2 is parent id column
			else
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, 5, intrackCat->selectedCatName, -1); // 2 is parent id column
		}
		else {
			// For updating parent categories from inventory_cat.c
			if(intrackCat->selectedCat == NULL) {
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, 2, "0", -1); // 2 is parent id column
			}
			else {
				gtk_tree_store_set(GTK_TREE_STORE(model), &iter, 2, intrackCat->selectedCat, -1); // 2 is parent id column
...
static void getCategories(IntrackCategories *intrackCat, gchar *selectedCat) {
	GtkListStore *store;
	
	store = gtk_list_store_new (CAT_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	pullCategories(store, selectedCat, intrackCat->b_inventory);
	
	gtk_tree_view_set_model(GTK_TREE_VIEW(intrackCat->catTree), GTK_TREE_MODEL(store));
	g_object_unref(store);
}

static int pullCategories(GtkListStore *store, gchar *selectedCat, gboolean b_inventory) {
	GtkTreeIter 	iter;
	...
	query_string = g_strconcat("SELECT a.display_name, a.id, a.section, b.label FROM `", CATEGORY_TABLES, "` a left join `", CATEGORY_SECTION, "` b on a.section = b.id ORDER BY id", NULL);
	...
	while ((row = mysql_fetch_row(result))) {
		gchar *catName, *id, *section, *section_id;
		
		for(i = 0; i < num_fields; i++) {
			if(i == 0)
				catName = g_strdup(row[i]);
		...
		// We want to show all categories
		if(b_inventory == TRUE) {
			gtk_list_store_append (store, &iter);
			gtk_list_store_set (store, &iter, CATNAME, catName, ID, id, SECTION, section, SECTION_ID, section_id, -1);			
		}
		else if(strcmp(selectedCat, id)) {
			// We dont want to display the currently selected category so we dont parent ourself.
			gtk_list_store_append (store, &iter);
			gtk_list_store_set (store, &iter, CATNAME, catName, ID, id, SECTION, section, SECTION_ID, section_id, -1);
		}
		
		g_free(catName), g_free(id), g_free(section), g_free(section_id);
	}	
...
static gboolean treeButtonPress(GtkWidget *widget, GdkEventButton *ev, IntrackCategories *intrackCat) {
    ...
    GtkTreeModel 	*model;
    GtkTreeIter 	iter;
    gchar *rowCat, *rowSection, *rowSectionName, *rowCatName;
	
	model = gtk_tree_view_get_model(GTK_TREE_VIEW(intrackCat->catTree));
    gtk_tree_model_get_iter(model, &iter, path);
    gtk_tree_model_get(model, &iter, ID, &rowCat, -1);
    gtk_tree_model_get(model, &iter, CATNAME, &rowCatName, -1);
    gtk_tree_model_get(model, &iter, SECTION_ID, &rowSection, -1);
    gtk_tree_model_get(model, &iter, SECTION, &rowSectionName, -1);
    ...
    intrackCat->selectedCat = g_strdup(rowCat);
    intrackCat->selectedCatName = g_strdup(rowCatName);
    intrackCat->selectedSection = g_strdup(rowSection);
    intrackCat->selectedSectionName = g_strdup(rowSectionName);
    
    g_free(rowCat), g_free(rowSection), g_free(rowSectionName), g_free(rowCatName);
...
static gboolean keyPressSelection(gpointer data) {
    ...
    gchar *rowtext, *rowsection, *rowsectionname, *rowcatname;
	...
	if(pathSelected) {
		...
		gtk_tree_model_get(model, &iter, ID, &rowtext, -1);
		gtk_tree_model_get(model, &iter, CATNAME, &rowcatname, -1);
		gtk_tree_model_get(model, &iter, SECTION_ID, &rowsection, -1);
		gtk_tree_model_get(model, &iter, SECTION, &rowsectionname, -1);
		...
		intrackCat->selectedCat = g_strdup(rowtext);
		intrackCat->selectedCatName = g_strdup(rowcatname);
		intrackCat->selectedSection = g_strdup(rowsection);
		intrackCat->selectedSectionName = g_strdup(rowsectionname);
		...
		g_free(rowtext), g_free(rowsection), g_free(rowsectionname), g_free(rowcatname);
	}
